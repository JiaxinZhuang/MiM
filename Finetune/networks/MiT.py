'''ECCV 2022 UniMiSS: Unified Medical Image Segmentation via Self-supervision
...
# import MiTnnu.network_architecture.MiT_encoder as MiT_encoder
...
class PatchEmbed_Encoder(nn.Module):
...
class MiT_encoder(nn.Module):
    ...
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', weight_std=False, img_size=[48, 192, 192], in_chans=1, num_classes=0, 
        ...
        self.patch_embed1 = PatchEmbed_Encoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=[img_size[0]//1, img_size[1]//2, img_size[2]//2], patch_size=[2, 2, 2], in_chans=32, embed_dim=embed_dims[0])
        self.patch_embed2 = PatchEmbed_Encoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=[img_size[0]//2, img_size[1]//4, img_size[2]//4], patch_size=[2, 2, 2], in_chans=embed_dims[0], embed_dim=embed_dims[1])
        self.patch_embed3 = PatchEmbed_Encoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=[img_size[0]//4, img_size[1]//8, img_size[2]//8], patch_size=[2, 2, 2], in_chans=embed_dims[1], embed_dim=embed_dims[2])
        self.patch_embed4 = PatchEmbed_Encoder(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=[img_size[0]//8, img_size[1]//16, img_size[2]//16], patch_size=[2, 2, 2], in_chans=embed_dims[2], embed_dim=embed_dims[3])
...
class MiT(nn.Module):
    ...
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', weight_std=False, img_size=[48, 192, 192], num_classes=None, in_chans=1,
        ...
        # Encoder
        if encoder=='small':
            self.transformer = model_small(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=img_size, in_chans=in_chans)
        elif encoder=='tiny':
            self.transformer = model_tiny(norm_cfg=norm_cfg, activation_cfg=activation_cfg, weight_std=weight_std, img_size=img_size, in_chans=in_chans)
...
class MiTnet(nn.Module):
    ...
    def __init__(self, norm_cfg='BN', activation_cfg='ReLU', weight_std=False, img_size=None, num_classes=None, in_chans=1,
        ...
        print(f'  + Number of Network Params: {total/1e6}M')

        if pretrain:
            pre_type = 'student'  #teacher student
            print(f'*********loading from checkpoint ssl: {pretrain_path}')
            ...
            if pre_type == 'teacher':
                ...
                print(f'Teacher: length of pre-trained layers: {len(pre_dict_ori)}')
            else:
                pre_dict_ori = torch.load(pretrain_path, map_location="cpu")[pre_type]
                pre_dict_ori = {k.replace("module.backbone.", ""): v for k, v in pre_dict_ori.items()}
                print(f'Student: length of pre-trained layers: {len(pre_dict_ori)}')

            pre_dict_ori = {k.replace("3D", ""): v for k, v in pre_dict_ori.items()}

            model_dict = self.model.state_dict()
            print(f'length of new layers: {len(model_dict)}')
            print(f'before loading weights: %.12f' % (self.model.state_dict()['transformer.block1.0.mlp.fc1.weight'].mean()))
