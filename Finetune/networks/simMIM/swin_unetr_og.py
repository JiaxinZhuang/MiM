        ...
        with torch.no_grad():
            ...
            if enc == 1:
                ...
                self.swinViT.layers1[0].downsample.reduction.weight.copy_(
                    weights["model"]["encoder.layers1.0.downsample.reduction.weight"]
                    # weights["model"]["encoder.layers1.0.0.downsample.reduction.weight"]
                )
                self.swinViT.layers1[0].downsample.norm.weight.copy_(
                    weights["model"]["encoder.layers1.0.downsample.norm.weight"]
                )
                self.swinViT.layers1[0].downsample.norm.bias.copy_(
                    weights["model"]["encoder.layers1.0.downsample.norm.bias"]
                )
                for bname, block in self.swinViT.layers2[0].blocks.named_children():
                    block.load_from(weights, n_block=bname, layer="layers2")
                self.swinViT.layers2[0].downsample.reduction.weight.copy_(
                    weights["model"]["encoder.layers2.0.downsample.reduction.weight"]
                )
                self.swinViT.layers2[0].downsample.norm.weight.copy_(
                    weights["model"]["encoder.layers2.0.downsample.norm.weight"]
                )
                self.swinViT.layers2[0].downsample.norm.bias.copy_(
                    weights["model"]["encoder.layers2.0.downsample.norm.bias"]
                )
                for bname, block in self.swinViT.layers3[0].blocks.named_children():
                    block.load_from(weights, n_block=bname, layer="layers3")
                self.swinViT.layers3[0].downsample.reduction.weight.copy_(
                    weights["model"]["encoder.layers3.0.downsample.reduction.weight"]
                )
                self.swinViT.layers3[0].downsample.norm.weight.copy_(
                    weights["model"]["encoder.layers3.0.downsample.norm.weight"]
                )
                self.swinViT.layers3[0].downsample.norm.bias.copy_(
                    weights["model"]["encoder.layers3.0.downsample.norm.bias"]
                )
                for bname, block in self.swinViT.layers4[0].blocks.named_children():
                    block.load_from(weights, n_block=bname, layer="layers4")
                self.swinViT.layers4[0].downsample.reduction.weight.copy_(
                    weights["model"]["encoder.layers4.0.downsample.reduction.weight"]
                )
                self.swinViT.layers4[0].downsample.norm.weight.copy_(
                    weights["model"]["encoder.layers4.0.downsample.norm.weight"]
                )
                self.swinViT.layers4[0].downsample.norm.bias.copy_(
                    weights["model"]["encoder.layers4.0.downsample.norm.bias"]
                )
...
class SwinTransformerBlock(nn.Module):
    ...
    def load_from(self, weights, n_block, layer):
        ...
        root = f"encoder.{layer}.0.blocks.{n_block}."
        block_names = [
            "norm1.weight",
            "norm1.bias",
