...
def get_metric(args):
    '''Get metric function.'''
    if args.metric_name == 'DSC':
        acc = DiceMetric(include_background=False, reduction=MetricReduction.MEAN, get_not_nans=False)
    elif args.metric_name == 'ACC':
        acc = Accuracy(args)
    else:
        print('No such metric', flush=True)
        sys.exit(-1)
    return acc
...
class Accuracy:
    ...
    def __call__(self, y, y_pred):
        ...
        self.preds_prob.extend(probabilities)
        pred = y_pred.argmax(dim=1)
        self.preds.extend(pred.cpu().numpy())
        self.gts.extend(y.cpu().numpy())
        # print(self.preds_prob)
        # print(self.preds)
        # print(self.gts)
    ...
    def aggregate(self):
        ...
        gts = torch.tensor(self.gts)
        preds = torch.tensor(self.preds)
        acc = torch.eq(gts, preds).float().mean()
        # self.preds_prob = self.preds_prob
        # ovr_auc = roc_auc_score(self.gts, self.preds_prob, average="micro", multi_class='ovr')
        # print(f"One-vs-Rest AUC-ROC: {ovr_auc:.4f}")
        return acc

    def reset(self):
        '''Reset the results.'''
        gts_onehot = [torch.eye(self.args.out_channels).cpu().numpy()[gt] for gt in self.gts]
        # print(self.gts, gts_onehot)
        ovr_auc = roc_auc_score(gts_onehot, self.preds_prob, average="micro", multi_class='ovr')
        print(f"One-vs-Rest AUC-ROC: {ovr_auc:.4f}")
        self.gts = []
        self.preds = []
        self.preds_prob = []
...
def compute_surface_distances(mask_gt, mask_pred, spacing_mm):
    ...
    #neighbour_code_to_surface_area = np.zeros(256, dtype=np.uint8)
...
def compute_surface_dice_at_tolerance(surface_distances, tolerance_mm):
    '''Compute the surface DICE score at a given tolerance.'''
...
class NSD:
    '''Compute the Normalized Surface Distance (NSD) between two masks.

    The NSD is defined as the average of the surface distances from the
    ground truth surface to the predicted surface and from the predicted
    surface to the ground truth surface. The surface distance is defined
    as the closest distance of each surface element to the other surface.
    ...
    Returns:
        The NSD between the two masks.

    '''

    def __init__(self, spacing_mm, tolerance_mm):
        self.tolerance_mm = tolerance_mm
        self.spacing_mm = spacing_mm
        self.scores = []

    def __call__(self, y_pred, y):
        mask_pred, mask_gt = y_pred, y
        surface_distance = compute_surface_distances(mask_gt, mask_pred, self.spacing_mm)
        pred = compute_surface_dice_at_tolerance(surface_distance, self.tolerance_mm)
        self.scores.append(pred)

    def aggregate(self):
        '''Return the NSD score.'''
...
def compute_robust_hausdorff(surface_distances, percent):
    '''Compute the robust Hausdorff distance at a given percentile.'''
    distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
    distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
    surfel_areas_gt      = surface_distances["surfel_areas_gt"]
    surfel_areas_pred    = surface_distances["surfel_areas_pred"]
...
class HD:
    '''Compute the Hausdorff Distance (HD) between two masks.

    The HD is defined as the maximum of the surface distances from the
    ground truth surface to the predicted surface and from the predicted
    surface to the ground truth surface. The surface distance is defined
    as the closest distance of each surface element to the other surface.
    ...
    Returns:
        The HD between the two masks.

    '''

    def __init__(self, spacing_mm, percent):
    ...
    def __call__(self, y_pred, y):
        mask_pred, mask_gt = y_pred, y
        surface_distance = compute_surface_distances(mask_gt, mask_pred, self.spacing_mm)
        pred = compute_robust_hausdorff(surface_distance, self.percent)
        self.scores.append(pred)
