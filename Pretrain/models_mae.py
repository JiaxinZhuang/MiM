'''use timm's vision transformer as backbone'''
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(self, x, mask_ratio):
        ...
        x: [N, L, D], sequence
        x_masked: [N, valid_len, D]
        mask: [N, L], 0 keep 1 remove
        ids_restore: [N, L], unshuffle
        """
        ...
        ids_shuffle = torch.argsort(noise, dim=1)  # ascend: small is keep, large is remove,  idx->shuffle
        ids_restore = torch.argsort(ids_shuffle, dim=1)                                     # shuffle->idx

        # keep the first subset
        ids_keep = ids_shuffle[:, :len_keep]
        x_masked = torch.gather(x, dim=1, index=ids_keep.unsqueeze(-1).repeat(1, 1, D))     # get valid patch
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        # add pos embed w/o cls token, [B, N, C] + [B, N, C]
        ...
        # apply Transformer blocks, [B, 1+N, D] -> [B, 1+N, D]
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        # add pos embed
        # print('x', x.shape)
        # print('decoder_pos_embed', self.decoder_pos_embed.shape)
...
def mae_vit_tiny_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=192, in_chans=1, depth=12, num_heads=3,
        decoder_embed_dim=576, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

# modify decoder_embed_dim from 512 -> 768 for 3d sincos encoding
# modify decoder_embed_dim from 512 -> 576 for 3d sincos encoding, divide by 2, 3, 16
...
def mae_vit_large_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=1152, in_chans=1, depth=24, num_heads=16,
...
def mae_vit_huge_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=1344, in_chans=1, depth=32, num_heads=16,
        decoder_embed_dim=768, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
