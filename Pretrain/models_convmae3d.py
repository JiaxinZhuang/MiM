...
class MaskedAutoencoderConvViT3d(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, (patch_size[0] * patch_size[1] * patch_size[2])**3 * in_chans, bias=True) # decoder to patch
        # --------------------------------------------------------------------------
    ...
    def initialize_weights(self):
        ...
        #TODO.
        pos_embed = get_3d_sincos_pos_embed(self.pos_embed.shape[-1], int(np.cbrt(self.patch_embed3.num_patches)), cls_token=False)
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        decoder_pos_embed = get_3d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], int(np.cbrt(self.patch_embed3.num_patches)), cls_token=False)
...
#        torch.nn.init.normal_(self.cls_token, std=.02)
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        # mask for stage 1 and 2
        mask_for_patch1 = mask.reshape(-1, 6, 6, 6).unsqueeze(-1).repeat(1, 1, 1, 1, 4*4*4).reshape(-1, 6, 6, 6, 4, 4, 4).permute(0, 1, 4, 2, 5, 3, 6).reshape(x.shape[0], 24, 24, 24).unsqueeze(1)
        mask_for_patch2 = mask.reshape(-1, 6, 6, 6).unsqueeze(-1).repeat(1, 1, 1, 1, 2*2*2).reshape(-1, 6, 6, 6, 2, 2, 2).permute(0, 1, 4, 2, 5, 3, 6).reshape(x.shape[0], 12, 12, 12).unsqueeze(1)
...
def convmae_convvit_3d_base_patch16_dec512d8b(**kwargs):
    ''' 3d convvit with input size 96x24x12'''
    model = MaskedAutoencoderConvViT3d(
        ...
        decoder_depth=8, 
        decoder_num_heads=16, mlp_ratio=[4, 4, 4], 
