...
class Debug(Transform):
    def __call__(self, data):
        # print('Debug', data['name'], data['image'].shape, data['label'].shape, data['post_label'].shape)
        d = data
        tmp = data['post_label']
        for index in range(32):
            print('Debug', d['name'], index, torch.sum(torch.tensor(tmp[index])), torch.sum(torch.tensor(d['label']) == (index+1)), tmp[index].shape, d['label'].shape)
        sys.exit(-1)
        return data
...
class RandCropByPosNegLabeld_select(RandCropByPosNegLabeld):
    def __call__(self, data):
        ...
        name = d['name']
        # for index in range(32):
            # print('randomcroplabel', name, index,
                #   torch.unique(d['post_label'][index]),
                #   d['post_label'][index].shape,
                #   torch.sum(d['post_label'][index]))
        # sys.exit(-1)
        # print(torch.unique(data['label']), data['label'].shape)
        # data['label'] = data['label'].type(torch.int32)
        key = get_key(name)
        if key in ['10_03', '10_07', '10_08', '04']:
            return d

        # print(d['label'].shape, d['image'].shape)
        # sys.exit(-1)
        # , d['post_label_0'].shape)
        # [NUM_CLAESSES, h, w, d]
        tmp = deepcopy(d['post_label'])
        for index in range(32):
            key_name = 'post_label_{}'.format(index)
            d[key_name] = deepcopy(torch.tensor(tmp[index]).unsqueeze(0))
            # print('btmp', index, torch.unique(d[key_name]))
            # if key_name not in self.keys:
                # self.keys.append(key_name)
            #TODO.....
            # print(index, name, torch.sum(torch.tensor(tmp[index])), torch.sum(torch.tensor(d['label']) == (index+1)), tmp[index].shape, d['label'].shape)
        # sys.exit(-1)

        # print('bxxx', torch.unique(d['post_label_0']), d['post_label_0'].shape)
        # for index in range(32):
            # print('xxx', torch.unique(d['post_label_{}'.format(index)]),
                #   d['post_label_{}'.format(index)].shape)
        d = super().__call__(d)
        # for index in range(32):
            # print('xxx', torch.unique(d[0]['post_label_{}'.format(index)]),
                #   d[0]['post_label_{}'.format(index)].shape, torch.unique(d[0]['label']))
        # sys.exit(-1)
        # print(d)
        # d = dict(d)
        # print(d)
        ...
        for idx, item in enumerate(d):
            # print(item.keys())
            ret = torch.zeros((32, 96, 96, 96))
            for index in range(32):
                key_name = 'post_label_{}'.format(index)
                # print('atmp', index, torch.unique(item[key_name]))
                ret[index] = deepcopy(item[key_name])
                # print('ttmp', index, torch.unique(tmp))
                # print('ttmp', index, tmp.shape)
                # ret[index] = tmp
                # tmp = ret
                # print(idx, torch.sum(tmp[index]), torch.sum(torch.tensor(d[idx]['label']) == (index+1)), tmp[index].shape, d[idx]['label'].shape)
            d[idx]['post_label'].set_array(ret)
            # print(idx, torch.unique(d[idx]['label']), torch.unique(d[idx]['post_label']))
        # sys.exit(-1)
        # print('tmp', torch.unique(d[0]['post_label']), d[0]['post_label'].shape)
        return d
...
class RandCropByLabelClassesd_select(RandCropByLabelClassesd):
    def __call__(self, data):
        ...
        if key not in ['10_03', '10_07', '10_08', '04']:
            return d
        d = super().__call__(d)
        # print('randomcropclass', name, torch.unique(data['post_label']))
        return d
...
def get_loader(args):
    special_keys = ['image', 'label']
    for idx in range(32):
        key_name = 'post_label_{}'.format(idx)
    ...
    if args.phase == 'train':
        ...
        train_sampler = DistributedSampler(dataset=train_dataset, even_divisible=True, shuffle=True) if args.distributed else None
