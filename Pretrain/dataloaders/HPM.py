...
class RandomMaskingGenerator:
    ''' Generate a random binary mask for a given image size.'''
    def __init__(self, input_size=6, mask_ratio=0.6, regular=False, block=False):
...
class MaskTransform(MapTransform):
    ''' Transform for masked pretraining.'''
    ...
    def __repr__(self):
        repr_str = "(MaskTransform,\n"
        repr_str += f"  transform = {str(self.transform)},\n"
        repr_str += f"  Masked position generator = {str(self.masked_position_generator)},\n"
        repr_str += ")"
        return repr_str
